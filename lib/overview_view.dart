  ...
  Widget build(BuildContext context) {
    return Consumer<UserService>(
      builder: (context, userService, child) {
        ...
        if (userService.role == UserRole.schoolAdmin) {
          return const SchoolAdminDashboard();
        } else if (userService.role == UserRole.lecturer && userService.lecturer != null) {
          return LecturerOverviewDashboard(lecturer: userService.lecturer!);
        } else if (userService.role == UserRole.student && userService.student != null) {
...
// --- SCHOOL ADMIN DASHBOARD (REFACTORED) ---
class SchoolAdminDashboard extends StatefulWidget {
  const SchoolAdminDashboard({super.key});
  @override
  State<SchoolAdminDashboard> createState() => _SchoolAdminDashboardState();
...
class _SchoolAdminDashboardState extends State<SchoolAdminDashboard> {
  ...
  Future<void> _fetchOptionsForLevel() async {
    if (!mounted) return;
    setState(() { _isFilterLoading = true; _currentOptions = []; });
    final items = await _userService.getFilterOptions(_currentLevel, _filterBreadcrumbs);
    if (mounted) setState(() { _currentOptions = items; _isFilterLoading = false; });
  }
  
  Future<void> _fetchChartData({bool forceRefresh = false}) async {
    if (_filterBreadcrumbs.isEmpty) return;
    setState(() { _isLoading = true; _studentGradeList.clear(); _touchedIndex = -1; });
    final data = await _userService.getChartData(_filterBreadcrumbs, forceRefresh: forceRefresh);
    if (mounted) {
      setState(() {
        _gradeData = data;
        _chartTitle = "Grades: ${_filterBreadcrumbs.lastOrNull?.name ?? 'Overview'}";
        _isLoading = false;
      });
    }
  }
  ...
  Future<void> _fetchStudentsForGrade(String grade) async {
    setState(() => _isStudentListLoading = true);
    final studentNames = await _userService.getStudentsForGrade(grade, _filterBreadcrumbs);
    if(mounted) setState(() { _studentGradeList = studentNames; _isStudentListLoading = false; });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: RefreshIndicator(
        onRefresh: () => _fetchChartData(forceRefresh: true),
        child: SingleChildScrollView(
          physics: const AlwaysScrollableScrollPhysics(),
          padding: const EdgeInsets.all(16.0),
          child: Column(
...
// --- DrillDownFilter Widget (NO CHANGES) ---
class _DrillDownFilter extends StatefulWidget {
  const _DrillDownFilter({
    required this.breadcrumbs,
    required this.options,
...
// --- Indicator Widget for Chart (NO CHANGES) ---
class Indicator extends StatelessWidget {
  const Indicator({ super.key, required this.color, required this.text, this.size = 16 });
  final Color color;
  final String text;
...
// --- StudentAcademicRecordView (NO CHANGES) ---
class StudentAcademicRecordView extends StatefulWidget {
  final Student student;
  const StudentAcademicRecordView({super.key, required this.student});
  @override
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  ...
                  child: ExpansionTile(
                    ...
                    subtitle: Text(record.courseCode),
                    children: [
                      ListTile(dense: true, leading: const Icon(Icons.calendar_today, size: 20), title: const Text('Enrolled'), subtitle: Text(DateFormat.yMMMd().format(record.enrollmentDate))),
                      ListTile(dense: true, leading: const Icon(Icons.person_outline, size: 20), title: const Text('Lecturer'), subtitle: Text(record.lecturerName)),
                      ListTile(dense: true, leading: const Icon(Icons.star_border, size: 20), title: const Text('Grade'), subtitle: Text(record.grade)),
                    ],
                  ),
                );
              },
...
class _ClassRosterPageState extends State<ClassRosterPage> {
  ...
  void initState() {
    super.initState();
    _rosterFuture = Provider.of<UserService>(context, listen: false).getClassRoster(widget.offeringId);
  }

  Future<void> _launchEmailToStudent(Student student) async {
    final encodedSubject = Uri.encodeComponent(widget.courseTitle);
    final Uri emailLaunchUri = Uri.parse('mailto:${student.email}?subject=$encodedSubject');
    if (await canLaunchUrl(emailLaunchUri)) {
      await launchUrl(emailLaunchUri);
    } else {
      Fluttertoast.showToast(msg: "Could not launch email client.", backgroundColor: Colors.red);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<Student>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
          if (snapshot.hasError) return Center(child: Text("Error fetching roster: ${snapshot.error}"));
          if (!snapshot.hasData || snapshot.data!.isEmpty) return const Center(child: Text("No students are enrolled in this course."));
...
// --- NewAnnouncementPage (NO CHANGES) ---
class NewAnnouncementPage extends StatefulWidget {
  final String offeringId;
  final String courseTitle;
  const NewAnnouncementPage({super.key, required this.offeringId, required this.courseTitle});
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                controller: _announcementController,
                decoration: const InputDecoration(labelText: 'Announcement Message', border: OutlineInputBorder()),
                maxLines: 5,
                validator: (value) {
                  if (value == null || value.trim().isEmpty) return 'Announcement cannot be empty';
                  return null;
                },
              ),
              const SizedBox(height: 20),
...
class _LecturerOverviewDashboardState extends State<LecturerOverviewDashboard> {
  ...
  void _fetchData({bool forceRefresh = false}) {
    setState(() {
      _teachingLoadFuture = _userService.getTeachingLoad(forceRefresh: forceRefresh);
      _announcementsFuture = _userService.getRecentAnnouncements(forceRefresh: forceRefresh);
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: RefreshIndicator(
        onRefresh: () async => _fetchData(forceRefresh: true),
        child: SingleChildScrollView(
          physics: const AlwaysScrollableScrollPhysics(),
          padding: const EdgeInsets.all(16.0),
          child: Column(
              ...
              FutureBuilder<List<LecturerCourseInfo>>(
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) => CourseInfoCard(
                      courseInfo: snapshot.data![index],
                      onDataChanged: () => _fetchData(forceRefresh: true),
                    ),
                  );
                },
              ),
